export default async function({ addon, global, console, msg }) {
  ...
  async function fetchNewPosts() {
    ...
    let res = await fetch(`https://scratch.mit.edu/discuss/topic/${topicId}/?page=${maxPage}`).then(res => res.text())
    
    let dom = (new DOMParser()).parseFromString(res, 'text/html');
    
    // load unfound posts
    let oldPosts = Array.from(document.querySelectorAll(".blockpost.roweven.firstpost")).map(el => Number(el.id.substring(1)))
