  ...
  async function createDropdown() {
    ...
    function createDropdownItem(name, click) {
      ...
      item.addEventListener("click", () =>
        window.open(
          click(addon.tab.redux.state.preview.projectInfo.id, username, addon.tab.redux.state.preview.projectInfo.title)
        )
      );
    ...
    table.appendChild(
      createDropdownItem(
        "HTMLifier",
        (projectId, username, title) => `https://sheeptester.github.io/htmlifier/?title=${title}&id=${projectId}`
      )
    );
    table.appendChild(
      createDropdownItem(
        "LeapordJS (JavaScript)",
        (projectId) => `https://leapordjs.vercel.app/api/${projectId}/codesandbox`
      )
    );
    table.appendChild(
      createDropdownItem("TurboWarp Packager", (projectId) => `https://packager.turbowarp.org/?id=${projectId}`)
    );
